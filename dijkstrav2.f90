...
!
  ...
  integer ( kind = 4 ) j
  integer ( kind = 4 ) initnode
  integer ( kind = 4 ) mind(nv),mindsort(nv),indx(nv)
...
!
  print *,'Please select the initial node:'
  read(*,*) initnode
  call dijkstra_distance ( initnode, nv, ohd, mind )
!
!  Print the results.
!
  write ( *, '(a)' )  ' '
!  write ( *, '(a)' )  '  Minimum distances from node 1:'
  write ( *, '(a,i3,a)' )  '  Minimum distances from node',initnode,' :'
  write ( *, '(a)' )  ' '
  do i = 1, nv
...
!
  ...
  call indexx(nv,mindsort,indx)
  do i = 1, nv
    write ( *, '(2x,i2,2x,i2,2x,i2,2x,i2)' ) i, mind(i),mindsort(i),indx(i)
  end do
...
subroutine dijkstra_distance ( initnode, nv, ohd, mind )
...
!
  ...
  integer ( kind = 4 ) ohd(nv,nv)
!
!  Start out with node, initnode, connected to the tree.
!
  connected(1:nv) = .false.
  connected(initnode) = .true.
!
!  Initialize the minimum distance to the one-step distance.
!
  mind(1:nv) = ohd(initnode,1:nv)
!
